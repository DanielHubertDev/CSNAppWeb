...
function createUserCollection(user){
   firebase.firestore().collection('userweb')
   .doc(user.uid)
   .set({
       uid:user.uid,
            ...
            if(doc.exists){
                 const userInfo = doc.data()
                    if(userInfo){
                        ...
                        <a href="utilisateur.html"><button class="btn waves-effect #fbc02d green darken-2 modal-trigger" >Utilisateurs</button></a>
...
function uploadImage(e){
    ...
    const fileRef = firebase.storage().ref().child(`/userweb/${uid}/profile`)
...
async function allUserDetails(){
  ...
  const userRef = await firebase.firestore().collection('userweb').get()  
